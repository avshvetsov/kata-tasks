...
public class TelemetryClient implements Closeable {
    ...
    public TelemetryClient(String telemetryServerConnectionString, int repeats) {
        while (connect(telemetryServerConnectionString) && repeats > 0) {
            repeats -= 1;
        }
    }

    private boolean connect(String telemetryServerConnectionString) {
